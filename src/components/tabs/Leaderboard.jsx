...
export default function Leaderboard() {
  ...
  const maxHours = board.length > 0 ? board[0].rawHours : 1;
    ...
    <div className="p-5 max-w-[1000px] mx-auto space-y-4">
      ...
      <div className="bg-white border border-border rounded-xl overflow-hidden">
        <table className="w-full text-sm">
          ...
          <tbody>
            {board.map((c, i) => {
              const barPct = maxHours > 0 ? (c.rawHours / maxHours) * 100 : 0;
              const medals = ['🥇', '🥈', '🥉'];
              return (
                <tr key={c.user} className="border-b border-border/50 hover:bg-bg transition-colors">
                  ...
                  <td className="px-4 py-3 text-right text-accent font-semibold">{c.rawHours.toFixed(2)}</td>
